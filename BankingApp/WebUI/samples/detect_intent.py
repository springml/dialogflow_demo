...
def _create_session_path(project_id, session_id):
    # Agent ID must be the same as project ID for v1alpha.
    return ConversationServiceClient.session_path(
        project=project_id, agent=project_id, session=session_id)


def _detect_text_intent(project_id, session_id, text, language_code):
